...
where
    D: TyDecoder,
{
    Ok(decoder.interner().tcx.arena.alloc(Decodable::decode(decoder)?))
}
...
where
    D: TyDecoder,
{
    Ok(decoder.interner().tcx.arena.alloc_from_iter(<Vec<T> as Decodable<D>>::decode(decoder)?))
}
...
impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> Decodable<D> for Ty<'tcx> {
    ...
    fn decode(decoder: &mut D) -> Result<Ty<'tcx>, D::Error> {
        ...
        if decoder.positioned_at_shorthand() {
            ...
            Ok(tcx.mk_ty(rustc_type_ir::TyKind::decode(decoder)?))
        }
    }
}
...
impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> Decodable<D> for ty::Predicate<'tcx> {
    fn decode(decoder: &mut D) -> Result<ty::Predicate<'tcx>, D::Error> {
        ...
        let predicate = decoder.interner().tcx.mk_predicate(predicate_kind);
        Ok(predicate)
    }
}
...
impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> Decodable<D> for mir::Place<'tcx> {
    fn decode(decoder: &mut D) -> Result<Self, D::Error> {
        ...
        let projection: &'tcx List<mir::PlaceElem<'tcx>> =
            decoder.interner().tcx.mk_place_elems((0..len).map(|_| Decodable::decode(decoder)))?;
...
impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> Decodable<D> for ty::Region<'tcx> {
    fn decode(decoder: &mut D) -> Result<Self, D::Error> {
        Ok(decoder.interner().tcx.mk_region(Decodable::decode(decoder)?))
    }
}
...
impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> Decodable<D> for CanonicalVarInfos<'tcx> {
    fn decode(decoder: &mut D) -> Result<Self, D::Error> {
        ...
        Ok(decoder.interner().tcx.intern_canonical_var_infos(interned?.as_slice()))
    }
}

impl<D: TyDecoder<I = TyInterner<'tcx>>> Decodable<D> for AllocId {
    fn decode(decoder: &mut D) -> Result<Self, D::Error> {
...
impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> Decodable<D> for ty::SymbolName<'tcx> {
    fn decode(decoder: &mut D) -> Result<Self, D::Error> {
        Ok(ty::SymbolName::new(decoder.interner().tcx, &decoder.read_str()?))
    }
}
...
impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> RefDecodable<'tcx, D> for ty::AdtDef {
    fn decode(decoder: &mut D) -> Result<&'tcx Self, D::Error> {
        let def_id = <DefId as Decodable<D>>::decode(decoder)?;
        Ok(decoder.interner().tcx.adt_def(def_id))
    }
}
...
impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> RefDecodable<'tcx, D> for ty::List<Ty<'tcx>> {
    fn decode(decoder: &mut D) -> Result<&'tcx Self, D::Error> {
        let len = decoder.read_usize()?;
        decoder.interner().tcx.mk_type_list((0..len).map(|_| Decodable::decode(decoder)))
    }
}
...
impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> RefDecodable<'tcx, D> for ty::Const<'tcx> {
    fn decode(decoder: &mut D) -> Result<&'tcx Self, D::Error> {
        Ok(decoder.interner().tcx.mk_const(Decodable::decode(decoder)?))
    }
}

impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> RefDecodable<'tcx, D> for [ty::ValTree<'tcx>] {
    fn decode(decoder: &mut D) -> Result<&'tcx Self, D::Error> {
        Ok(decoder.interner().tcx.arena.alloc_from_iter(
...
impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> RefDecodable<'tcx, D> for Allocation {
    fn decode(decoder: &mut D) -> Result<&'tcx Self, D::Error> {
        Ok(decoder.interner().tcx.intern_const_alloc(Decodable::decode(decoder)?))
    }
}
...
impl<'tcx, D: TyDecoder<I = TyInterner<'tcx>>> RefDecodable<'tcx, D>
    for [mir::abstract_const::Node<'tcx>]
{
    fn decode(decoder: &mut D) -> Result<&'tcx Self, D::Error> {
        Ok(decoder.interner().tcx.arena.alloc_from_iter(
            (0..decoder.read_usize()?)
    ...
    for ty::List<ty::BoundVariableKind>
{
    fn decode(decoder: &mut D) -> Result<&'tcx Self, D::Error> {
        let len = decoder.read_usize()?;
        decoder.interner().tcx.mk_bound_variable_kinds((0..len).map(|_| Decodable::decode(decoder)))
    }
}
